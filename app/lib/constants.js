...
// Mapping from Shopify ISO 3166-1 alpha-2 country codes to our internal codes
// Shopify Admin API returns 2-letter codes (e.g., "GB", "PK"), we use 3-letter or custom codes
export const SHOPIFY_COUNTRY_CODE_MAP = {
  ...
  'CI': 'CIV', 'DK': 'DNK', 'DJ': 'DJI', 'DM': 'DMA', 'DO': 'DOM',
  ...
  'KR': 'KOR', 'XK': 'XKX', 'KW': 'KUWAIT', 'KG': 'KGZ', 'LA': 'LAO',
  ...
  'PL': 'POL', 'PT': 'PRT', 'QA': 'QATAR', 'RO': 'ROU', 'RU': 'RUS',
  'RW': 'RWA', 'KN': 'KNA', 'LC': 'LCA', 'VC': 'VCT', 'WS': 'WSM',
  'SM': 'SMR', 'ST': 'STP', 'SA': 'KSA', 'SN': 'SEN', 'RS': 'SRB',
  ...
  'AE': 'UAE', 'GB': 'GBR', 'US': 'USA', 'UY': 'URY', 'UZ': 'UZB',
  'VU': 'VUT', 'VA': 'VAT', 'VE': 'VEN', 'VN': 'VNM', 'YE': 'YEM',
  'ZM': 'ZMB', 'ZW': 'ZWE',
};

/**
 * Convert Shopify ISO 2-letter country code to our internal code
 * @param {string} shopifyCode - ISO 3166-1 alpha-2 code (e.g., "GB", "PK")
 * @returns {string} - Our internal code (e.g., "GBR", "PAK") or "PAK" as fallback
 */
export function mapShopifyCountryCode(shopifyCode) {
  if (!shopifyCode) return 'PAK';
  return SHOPIFY_COUNTRY_CODE_MAP[shopifyCode.toUpperCase()] || 'PAK';
}
...
/**
 * Get cities for a country. Returns [] when no city list is defined, in which
 * case the city field renders as a free-text input (same pattern as provinces).
 */
